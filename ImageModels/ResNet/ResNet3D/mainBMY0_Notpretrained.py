...
if __name__ == "__main__":
    ...
    fold_metrics_df = pd.DataFrame(columns=['Fold', 'AUC', 'Accuracy', 'Bal_accuracy', 'Specificity', 'Precision', 'Recall', 'F1-score' 'NPV'])
    ...
    for fold in range(1, n_folds):
        ...
        if config_args.train:
            ...
            model = resnet34(spatial_dims=3, n_input_channels=1, pretrained=True,
                           num_classes=1, feed_forward=False,
                           shortcut_type="A", bias_downsample=False)
            model.to(device)
