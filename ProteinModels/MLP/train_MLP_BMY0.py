...
path_to_save_results = '/home/ubuntu/tenerife/data/LungAmbition/Proteinas/ResultsBaselines_BMY0/MLP'
...
name_file= f"MLP_proteins_6march25_{n_folds}folds"
...
for fold in range(0, n_folds):
    ...
    # train MLP classifier, first find the best hyperparameters through a grid search cv,
    # then train the model with the best hyperparameters
    # parameters to search
    param_grid = {
        'hidden_layer_sizes': [
            (512, 256, 128, 64, 32, 16),
            (512, 256, 128, 64, 32),  # Deep architecture
            (256, 128, 64, 32),  # Medium complexity
            (256, 64, 16),  # Smaller network
        ],
        'activation': ['relu'],
        'solver': ['adam'],
        'alpha': [0.5, 0.1, 0.05],  # Regularization
    ...
    y_train_pred_prob = best_mlp.predict_proba(X_train)
    ...
    y_pred_prob = best_mlp.predict_proba(X_test)
    # predict classes
    ...
    if keep_false_positives_as_separate_test:
        ...
        # apply PCA to X_false_positives
        # X_false_positives = pca.transform(X_false_positives)
        # X_false_positives = pd.DataFrame(X_false_positives, columns=columns)
        # print("Dimensions X_false_positives after PCA:", X_false_positives.shape)
        y_false_positives_pred_prob = best_mlp.predict_proba(X_false_positives)
        y_false_positives_pred = best_mlp.predict(X_false_positives)
    ...
    perm_importance_result = permutation_importance(best_mlp, X_train, y_train, n_repeats=100, random_state=1, n_jobs=-1)
    ...
    # Combining both importances for top 30 features
    top_features_perm = pd.DataFrame({
        'Feature': X_train.columns[perm_indices][:100],
        'Permutation Importance': perm_importances[perm_indices][:100],
        'Rank': range(1, 101)
    })
    top_features_perm['Fold'] = fold

    # Plotting Permutation Feature Importances
    plt.figure()
    plt.title(f"Permutation Feature Importances fold {fold + 1}")
    plt.bar(range(30), perm_importances[perm_indices][:30])
    plt.xticks(range(30), X_train.columns[perm_indices][:30], rotation=90)
    plt.tight_layout()
    plt.savefig(os.path.join(path_to_save_plots, f"Permutation_Feature_importances_fold_{fold + 1}.png"))

    top_features_df_list.append(top_features_perm)
