...
name_file= "ROS_RSF_proteins_17march25_BMY012"
...
fold_metrics_df = pd.DataFrame(columns=['Fold', 'Concordance_Index', 'Integrated_Brier_Score',
                                        'AUC_at_0_Years', 'Accuracy_at_0_Years', 'Balanced_Accuracy_at_0_Years', 'Precision_at_0_Years', 'Recall_at_0_Years', 'F1_Score_at_0_Years', 'Specificity_at_0_Years', 'NPV_at_0_Years', 'PPV_at_0_Years',
                                        'AUC_at_1_Years', 'Accuracy_at_1_Years', 'Balanced_Accuracy_at_1_Years', 'Precision_at_1_Years', 'Recall_at_1_Years', 'F1_Score_at_1_Years', 'Specificity_at_1_Years', 'NPV_at_1_Years', 'PPV_at_1_Years',
                                        'AUC_at_2_Years', 'Accuracy_at_2_Years', 'Balanced_Accuracy_at_2_Years', 'Precision_at_2_Years', 'Recall_at_2_Years', 'F1_Score_at_2_Years', 'Specificity_at_2_Years', 'NPV_at_2_Years', 'PPV_at_2_Years',])
...
for file_path in sorted(glob.glob(os.path.join(path_to_fold_division_folder, 'id2splitfold_*.csv'))):
    ...
    # filter in df_fold only rows where TimeYears_CT_blood is 0, 1, 2
    df_fold = df_fold[(df_fold['TimeYears_CT_blood'].isin([0, 1, 2])) | (df_fold['Group'] == 'Benign_Nodules') | (df_fold['split'] == 'test_false_positive')]
    ...
    df_fold.loc[(df_fold['TimeYears_CT_blood'] == 5) & (df_fold['Group'] != 'Lung_Cancer'), 'TimeYears_CT_blood'] = 3
    print("=" * 80)
    print(f"Fold {fold + 1}:")
    ...
    param_grid = {
        ...
        "max_features": ['sqrt', 150, 250, 'log2'],
    }
    ...
    plt.savefig(os.path.join(path_to_save_plots, f"KM_Censoring_Surv_{name_file}_fold_{fold+1}.png"))
    plt.close()

    # print("surv_funcs:", surv_funcs)
    # time_points = [0, 1, 2, 3, 4, 5]  # Specify the time points at which to calculate the Brier score
    time_points = [float(t) for t in [0, 1, 2]]
    ...
    for t in time_points:
        ...
        precision = precision_score(y_true_binary, y_pred_binary, average='binary')  # Handles cases with no positive predictions
        recall = recall_score(y_true_binary, y_pred_binary, average='binary')
        f1_sc = f1_score(y_true_binary, y_pred_binary, average='binary')
    ...
    fold_metrics_df = pd.concat([fold_metrics_df, pd.DataFrame([{
        ...
        'PPV_at_0_Years': performance_metrics[0]["PPV"],
        ...
        'PPV_at_1_Years': performance_metrics[1]["PPV"],
        ...
        'PPV_at_2_Years': performance_metrics[2]["PPV"],
    }])], ignore_index=True)
    # restart performance_metrics
    performance_metrics = []
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(path_to_save_plots, f"Permutation_feature_importances_{name_file}_fold_{fold+1}.png"))
    plt.close()
    print("*" * 80)
...
print("Mean and SD of Metrics at 0, 1, 2 years:")
for t in [0, 1, 2]:
    ...
    mean_PPV = fold_metrics_df[f"PPV_at_{t}_Years"].mean()
    std_PPV = fold_metrics_df[f"PPV_at_{t}_Years"].std()
    ...
    print(f"PPV - Mean: {mean_PPV:.3f}, SD: {std_PPV:.3f}")
    print("-" * 40)
